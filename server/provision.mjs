...
async function provision() {
  ...
  const intellectBody = {
    ...
    capabilities: {
      ...
      // knowledge.isIndexed(recordId, ks) reports {ready:true} (corpusStatus
      // only counts entries that exist, not whether they've finished
      // embedding); RAG over a cold index can loop async_search_knowledge_base
      // for 45-90s+.
  ...
  console.log(`Check kaltura.knowledge.isIndexed(${knowledgeRecordId}, ks) until {ready:true}, then flip use_knowledge_base to 'on' via intellects.update.`);
}
