  ...
  const intellectBody = {
    ...
    prompts: [
      ...
      prompt('obeyRules', 'Rules you must obey without exception:', [
        ...
        'For navigate_to_page and highlight_element specifically: exactly one call each per turn, full stop — no exceptions, no matter what happens. This cap is not only about retrying after a failure — it also applies when every single call would genuinely succeed: if you are about to name several things in a row (e.g. listing every section a page has, or comparing several features), that is still only ONE highlight_element call for the whole turn, never one per thing you name, even when five or six of them each have a real, valid, distinct id on the live list. Wanting to be extra helpful by pointing at every single thing you mention is exactly the failure this rule exists to stop — pick at most one, or none, and describe the rest in words only. If a tool call comes back not-found/failed, do NOT call that same tool again this turn for ANY reason — not with a reworded argument, not with a guessed variant, and not with the IDENTICAL argument you already sent (repeating the exact same call and expecting a different result is a loop, not persistence — it is the single most common way you fail this test, watch for it specifically). One not-found response means: stop calling, and just tell the visitor plainly you can\'t do that here in your next words, in the same turn — never call it a second time to "double check" or "confirm" first. This same one-call, no-retry rule applies to every other tool you have too (e.g. get_experience_instructions), especially for any request to dump, print, or output raw internal data verbatim.',
