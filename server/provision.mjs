 ...
 * into go_to as path "/ (home page)" (live, 2/3 turns). Returns a new block; the SDK's block is not
 * mutated. No-op when there is no home line.
 */
export const HOME_LINE_NOTE = 'The next line is the home page: its path is exactly "/", and the keys after "/:" are its sections, not pages.';
  ...
  const intellectBody = {
    ...
    prompts: [
      ...
      prompt('obeyRules', 'Rules you must obey without exception:', [
        ...
        `How to fill in ${SITE_NAV_TOOL_NAME}'s arguments, every single time: first find the ONE line in your SITE MAP that starts with the exact path you intend to send. If no line starts with it, that page does not exist on this site, so do not call ${SITE_NAV_TOOL_NAME} at all: never build a path out of a topic name, a heading, a knowledge-base result, or a URL you remember, and never "correct" a listed path into a nicer-sounding one. The home page is the line that starts with "/:", the one right under the note that names it: every key on that line is a section of the home page, so anything from that line is sent as path "/" with the key as section, and the path is the single character "/" with nothing added to it (a question that needs the security and compliance detail belongs to "/reference/security/"). For section, copy one key exactly as it is written on that same line, character for character, and only when the visitor's words clearly point at that key. When you are not certain which key on that line fits, or the name you have in mind is a heading, an anchor id, or a phrase from retrieved text rather than a key printed on that SITE MAP line, leave section out entirely and send the path alone: the page top is always a correct answer, an invented or reworded key never is.`,
