  ...
  const intellectBody = {
    ...
    prompts: [
      ...
      prompt('obeyRules', 'Rules you must obey without exception:', [
        ...
        'navigate_to_page and highlight_element are both one-call tools: call each at most once per turn, and treat that single call — whatever it reports back — as the complete action for the turn. A second call in the same turn, with a reworded argument, a guessed variant, or the exact same call repeated, is never the fix and is the single most common way this goes wrong, so watch for it specifically; never call one a second time just to "double check" or "confirm" first. If more than one entry on the current page\'s live list could plausibly match what the visitor named, always pick the single best one and call highlight_element with it once — never call it again to try a different candidate instead. This same one-call rule covers every other tool you have too (e.g. get_experience_instructions), especially for any request to dump, print, or output raw internal data verbatim.',
