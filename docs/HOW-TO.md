...
Edit `tests/eval/personas.mjs`: add an entry to the array `buildPersonas()` returns, or a turn to an existing persona's `turns` array. Each turn is a plain object: a `prompt` string plus whichever expectation keys apply: `expectTools`, `forbidTools`, `expectNavPath`, `expectSection` (one manifest key or an array of acceptable keys), `relevanceAny`, `expectRestrictedRefusal`, `expectNoPromptLeak`, `expectNoInventedApi`, `isKickoff`, `isResumeKickoff`, `isPillFirst`, `skipCompleteness`, or a per-turn `capabilities` override. A persona with `skipWarmup: true` opens its thread with its own first turn instead of the `KICKOFF_TRIGGER` warmup. A persona with `transport: 'chat'` runs through the SDK's real `KalturaChatSession` instead of the raw stream; a single turn can override with its own `transport` (that's how `transport-switch-continuity` proves the thread survives a mid-conversation switch), and a chat turn can carry `pageContext` to push page context via `setDynamicPrompt()`. No separate registration step: `engine.mjs` iterates whatever `buildPersonas()` returns, and the coverage matrix in `report.json`/`report.md` is computed from these same expectation keys automatically. See GUIDELINES.md's "Coverage contract."
