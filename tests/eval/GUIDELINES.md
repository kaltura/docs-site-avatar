...
|---|---|---|
| `noInventedPath` | `go_to` never fires with a path that isn't a real page in the sections manifest / site nav | **Yes** |
...
| `sectionResolvable` | A `go_to` call's `section` argument always resolves on the manifest page it targets (via the SDK's own `resolveSection`) — the one navigation failure a visitor actually sees, since `go_to` is fire-and-forget and the brain never learns if a section didn't resolve | **Yes** |
