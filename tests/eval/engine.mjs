 ...
 */
import { scoreTurn, toolNames, sitePath, DIMENSIONS } from './probes.mjs';
...
/**
 * Run one live conversation turn and record every tool call. Nova's only client tool (`go_to`)
 * is fire-and-forget, so nothing is ACKed — the brain never waits on the browser.
 * @param {{management: object, configId: number, message: string, threadId: string|null, capabilities?: object, transport?: 'stream'|'chat', pageContext?: object}} opts
 ...
 *   turn, the exact call the site makes per page (`{page:{title,url}}`).
 */
export async function runTurn({ management, configId, message, threadId, capabilities, transport = 'stream', pageContext }) {
  const t0 = Date.now();
  const ctrl = new AbortController();
  const turnFn = transport === 'chat' ? chatTurn : streamTurn;
  try {
    const r = await withTimeout(
      turnFn({ management, configId, message, threadId, capabilities, pageContext, signal: ctrl.signal }),
      TURN_TIMEOUT_MS,
      message,
      ctrl,
    );
    return {
      latencyMs: Date.now() - t0, threadId: r.threadId, text: r.text, toolCalls: r.toolCalls, error: null,
    ...
    return {
      latencyMs: Date.now() - t0, threadId, text: '', toolCalls: [], error: String(e?.detail || e?.message || e),
  ...
  for (let trial = 1; trial <= trials; trial++) {
    ...
    for (const p of personas) {
      ...
      const warmup = skipWarmup
        ? { threadId: null }
        : await runTurn({ management, configId, message: KICKOFF_TRIGGER, threadId: null, transport: personaTransport });
      ...
      for (const t of p.turns) {
        ...
        const r = await runTurn({ management, configId, message: t.prompt, threadId, capabilities: t.capabilities, transport: turnTransport, pageContext: t.pageContext });
        threadId = r.threadId || threadId;
        const rec = {
          prompt: t.prompt, expectation: t, latencyMs: r.latencyMs, text: r.text, toolCalls: r.toolCalls, error: r.error,
          spiralDetected: r.spiralDetected, spiralRecovered: r.spiralRecovered, rawToolSegCount: r.rawToolSegCount,
...
export function buildReport({ personaResults, personas, siteData, configId, trials = 1 }) {
  ...
  // Coverage is measured against the published sections manifest (the same file `go_to`
  // navigates against), not nav.js: a page Nova can't route to isn't a page she can show.
  const exercisedPaths = new Set(
    allTurns.flatMap((t) => t.toolCalls.filter((c) => c.name === 'go_to').map((c) => sitePath(c.args?.path, siteData.baseUrl)))
  );
  const uncoveredPages = (siteData.manifest?.pages || []).filter((p) => !exercisedPaths.has(sitePath(p.path)));
  const pagesTotal = siteData.manifest?.pages?.length ?? 0;
  ...
  const summary = {
    ...
    pagesTotal,
    pagesExercised: pagesTotal - uncoveredPages.length,
  ...
  return {
    _meta: { generatedAt: new Date().toISOString(), configId, siteDir: siteData.siteDir, pages: pagesTotal, trials },
    summary,
    coverage: {
      expectedTools: [...expectedTools],
      observedTools: [...observedTools],
      uncoveredPages: uncoveredPages.map((p) => p.path),
    },
