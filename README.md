...
Credentials live only in `.env` (gitignored) and, for CI, in secrets on the `production` GitHub Environment specifically — a repo-level secret isn't visible to those environment-gated jobs at all. Never commit a real `AGENTIC_ADMIN_SECRET` or a raw KS token. `server/agent.json` (the stable provisioned ids: intellect, avatar, agent, widget) IS committed — it's runtime state, not a secret, and CI needs it on disk to redeploy the same live intellect/agent/avatar instead of minting new ones every run. The knowledge corpus is discovered live from the intellect, so a redeploy never changes the file. See [docs/HOW-TO.md](docs/HOW-TO.md) for redeploying via GitHub Actions and setting up its secrets.
